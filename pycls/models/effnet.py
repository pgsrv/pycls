...
from pycls.core.net import init_weights, drop_connect
...
class MBConv(Module):
    ...
    def forward(self, x):
        f_x = x
        if self.exp:
            f_x = self.exp_af(self.exp_bn(self.exp(f_x)))
...
class EffStage(Module):
    ...
    def __init__(self, w_in, exp_r, k, stride, se_r, w_out, d):
        ...
        for i in range(d):
            ...
            name = "b{}".format(i + 1)
            self.add_module(name, MBConv(b_w_in, exp_r, k, b_stride, se_r, w_out))
    ...
    def complexity(cx, w_in, exp_r, k, stride, se_r, w_out, d):
        for i in range(d):
            b_stride = stride if i == 0 else 1
            b_w_in = w_in if i == 0 else w_out
            cx = MBConv.complexity(cx, b_w_in, exp_r, k, b_stride, se_r, w_out)
        return cx
...
class EffNet(Module):
    ...
    def __init__(self, params=None):
        ...
        for i, (d, w, exp_r, stride, k) in enumerate(stage_params):
            name = "s{}".format(i + 1)
            self.add_module(name, EffStage(prev_w, exp_r, k, stride, se_r, w, d))
            prev_w = w
